...
func (d *driver) NextDevice(
	...
	for localDevice := range localDeviceMapping {
		re, _ := regexp.Compile(`^/dev/` +
			fcUtils.NextDeviceInfo.Prefix +
			`(` + fcUtils.NextDeviceInfo.Pattern + `)`)
	...
	for _, letter := range letters {
		...
		return fmt.Sprintf(
			"/dev/%s%s", fcUtils.NextDeviceInfo.Prefix, letter), nil
	}
...
func (d *driver) getEphemeralDevices(
	...
	buf, err := fcUtils.BlockDevices(ctx)
	if err != nil {
		return nil, err
	}
	...
	for scanner.Scan() {
		...
		name, err := fcUtils.BlockDeviceName(ctx, string(word))
		if err != nil {
