...
func (d *driver) VolumeCreate(
	...
	volumeReq := &godo.VolumeCreateRequest{
		Region:        d.config.GetString(do.ConfigDORegion),
		Name:          name,
...
func (d *driver) VolumeRemove(
	ctx types.Context, volumeID string, opts *types.VolumeRemoveOpts) error {
	...
	if len(volume.DropletIDs) != 0 {
		return goof.New("volume still attached to a host")
	}
...
func (d *driver) VolumeAttach(
	...
	if len(vol.DropletIDs) > 0 {
		...
		_, _, err := d.client.StorageActions.Detach(volumeID)
		if err != nil {
			return nil, "", err
		}
	}
